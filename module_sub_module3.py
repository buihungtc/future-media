...
def loop_component_fixed(file_path, youtube_id, id_video, url_kenh):
    ...
    for loop_count in range(48):
        try:
            ...
            # Thêm vòng lặp retry cho comment_count
            comment_count = None
            max_retries = 3
            ...
            for retry in range(max_retries):
                try:
                    ...
                    if comment_count is not None:
                        ...
                        break  # Thoát vòng lặp nếu có giá trị hợp lệ
                    else:
                        print(f"⚠️ Lần thử {retry + 1}: Không phân tích được số bình luận.")

                except Exception as e:
                    print(f"❌ Lần thử {retry + 1} - Lỗi: {e}")
                
                # Nếu chưa thành công và chưa hết số lần thử
                if comment_count is None and retry < max_retries - 1:
                    print(f"🔄 Load lại trang và thử lại lần {retry + 2}")
                    driver.refresh()  # Load lại trang
                    sleep(3)  # Chờ trang load
                    driver.execute_script("window.scrollBy(0, window.innerHeight);")  # Lướt xuống 1 khung màn hình
                    sleep(2)  # Chờ một chút sau khi scroll

            # Nếu sau 3 lần vẫn None
            if comment_count is None:
                print("❌ Không thể lấy được số bình luận sau 3 lần thử")
